...
private:
  ...
  // -1 : Single sensor per robot, ignore sub-indexes (legacy mode)
  const char *mapName(const char *name, size_t robotID, Stg::Model* mod, int modelID = -1);
...
const char *
StageNode::mapName(const char *name, size_t robotID, Stg::Model* mod, int modelID)
{
...
StageNode::StageNode(int argc, char** argv, bool gui, const char* fname, bool use_model_names)
{
  ...
  if (lasermodels.size() > 0 && (lasermodels.size() % positionmodels.size() != 0))
  {
    // TODO: Change Error
...
StageNode::WorldCallback()
{
  ...
  for (size_t r = 0; r < this->positionmodels.size(); r++)
  {
    for (std::size_t l = 0; l < this->num_lasers_per_robot; l++)
    {
      ...
      Stg::Pose lp = this->lasermodels[index]->GetPose();
